...
namespace BlokusIA
{
    ...
    void GameState::generateValidMoves(Slot _player, bool _ignoreCornerRule, 
                                       const Board& _board, u32 _numSlots, const Board::PlayableSlots& _slots, const std::bitset<BlokusGame::PiecesCount>& _remainingPieces, 
    ...
    float GameState::computeScoreUpperBound(Slot _player, BoardHeuristic) const
    {
        ...
        do
        {
            // TODO: fill list of valid positions from clusterExpander

            generateValidMoves(Slot::P0, true, clusterExpander.generateBoard(Slot::P1), 0, {}, remainingPieces, moves, &bestFitScores);
            if (moves.size() > 0)
            {
            ...
            // TODO: add piece in clusterExpander
